...
import { NerdQAxeAdapterService } from './nerdqaxe-adapter.service';
...
export class SystemApiService {
  ...
  constructor(
    ...
    private adapter: NerdQAxeAdapterService,
  ...
  public getInfo(uri: string = ''): Observable<ISystemInfo> {
    // NerdQAxe Edition: fetch raw API response and map through adapter
    if (environment.production && !uri) {
      return this.httpClient.get<any>(`/api/system/info`).pipe(
        timeout(API_TIMEOUT),
        map(raw => this.adapter.mapSystemInfo(raw))
      );
    }

    if (environment.production && uri) {
      return this.httpClient.get<any>(`${uri}/api/system/info`).pipe(
        timeout(API_TIMEOUT),
        map(raw => this.adapter.mapSystemInfo(raw))
      );
    }
  ...
  public updateSystem(uri: string = '', update: any): Observable<void> {
    // NerdQAxe Edition: map LottoAxe field names to NerdQAxe field names
    const mappedUpdate = this.adapter.mapSettingsToNerdQAxe(update);

    if (environment.production && !uri) {
      return this.httpClient.patch<void>(`/api/system`, mappedUpdate);
    }

    if (environment.production && uri) {
      return this.httpClient.patch<void>(`${uri}/api/system`, mappedUpdate);
    }
