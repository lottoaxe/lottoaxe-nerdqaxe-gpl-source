...
export class TuningPresetsComponent implements OnInit {
  ...
  // Extended OC limits — DISABLED in NerdQaxe Edition (beta safety)
  ...
  // NerdQaxe Edition: beta safety — no aggressive presets
  readonly isNerdQaxeEdition = true;
  ...
  loadAsicSettings(): void {
    this.systemService.getAsicSettings().subscribe(asic => {
      ...
      // Select per-ASIC preset table, fall back to BM1368 (NerdQAxe+ default)
      let allPresets = ASIC_PRESETS[this.asicModel] || ASIC_PRESETS['BM1368'];

      // NerdQaxe Edition: filter out dangerous presets (YOLO OC, Lotto Mode)
      if (this.isNerdQaxeEdition) {
        allPresets = allPresets.filter(p => p.riskLevel !== 'extreme' && p.riskLevel !== 'high');
      }
  ...
  toggleOverclock(): void {
    // NerdQaxe Edition: overclocking is disabled in beta
    if (this.isNerdQaxeEdition) {
      return;
    }
